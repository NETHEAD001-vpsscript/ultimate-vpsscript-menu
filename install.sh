...
XRAY_UUID=$(uuidgen)
echo "$XRAY_UUID" > /etc/xray/server_uuid.txt
...
if [ ! -f /usr/local/bin/trojan-go ]; then
  mkdir -p /tmp/trojan
  # Replace with latest release URL from the official page
  curl -L -o /tmp/trojan/trojan-go.zip https://github.com/trojan-gfw/trojan-go/releases/download/v0.10.13/trojan-go-linux-amd64.zip
...
# ================== Finalize ==================
touch "$MARK_FILE"
echo "Installation and setup complete! Type 'menu' to manage your server."
